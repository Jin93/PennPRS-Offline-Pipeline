...
option_list = list(
  ...
  make_option("--methods", action = "store", default = 'C+T,lassosum2,LDpred2', type = "character",
              help="Options: a subset of methods from C+T, lassosum2, LDpred2, and PRS-CS, divided by comma"),
...
ld_path <- paste0(PennPRS_path,'/Files/')
PUMAS_path = paste0(PennPRS_path,'Files/code/')
...
if ('C+T' %in% methods){
  ...
  for (ite in 1:k){
    ...
    write_delim(SCORE, file = paste0(prsdir, trait_name,'.',method,'.ite',ite,".txt"), delim='\t')
  }
  rm(SCORE)
}
...
if (('lassosum2' %in% methods) | ('LDpred2' %in% methods)){
  ...
  for (ite in 1:k){
    ...
    if ('lassosum2' %in% methods){
      ...
      bigreadr::fwrite2(beta_lassosum2,file = output_lassosum2)
    ...
    if ('LDpred2' %in% methods){
      ...
      bigreadr::fwrite2(beta_ldpred2,file = output_LDpred2) #, delim='\t')
...
if ('C+T' %in% methods){
  ...
  for (ite in 1:k){
    ...
    write_delim(scores,file = paste0(input_path, trait_name,'.',method, '.ite',ite,'.txt'), delim='\t')
...
err.CT = err.lassosum2 = err.LDpred2 = err.PRSCS = 0
...
if ('C+T' %in% methods){
  ...
  if (nrow(beta_ct.out) > 0){
    ...
    write_delim(beta_ct.out, file = prs_ct_outputfile, delim='\t')
    ...
    rm(optimal.indx)
    write_delim(params.ct[ct.optimal.indx,], file = paste0(PennPRS_finalresults_path, trait_name,'.',method,'.optimal_params.txt'))
    if ( opt$verbose >= 1 ){
      print(paste0('*************************************'))
  ...
  if ('lassosum2' %in% methods){
    ...
    if (length(params.tuned) > 0){
      ...
      if (length(nonzero.indx) > 0){
        ...
        beta_lassosum2 = data.frame(df_beta[,c('chr','rsid','a1','a0')], beta_lassosum2[,train.indx.lassosum2[nonzero.indx][indx.temp]])
        ...
        if (nonzero){
          ...
          write_delim(beta_lassosum2.out, file = prs_lassosum2_outputfile, delim='\t')
    ...
    if (length(params.tuned) > 0){
      ...
      if (length(nonzero.indx) > 0){
        ...
        if (nonzero){
          ...
          write_delim(beta_ldpred2.out, file = prs_ldpred2_outputfile, delim='\t')
          print(paste0('Optimal parameter setting: '))
...
if ('PRS-CS' %in% methods){
  ...
  write_delim(sumraw[, c('SNP', 'A1', 'A2', 'BETA', 'SE')], prscs.sumdat.file, delim = '\t')
  ...
  if (nrow(beta_prscs.out) > 0){
    ...
    write_delim(beta_prscs.out, file = prs_prscs_outputfile, delim='\t')
    
    optimal.indx = params.tuned.prscs
    save(optimal.indx, file = paste0(prsdir, trait_name, '.', method, '.optimal.indx.RData'))
    rm(optimal.indx)
    write_delim(data.frame(phi = phi.vals[params.tuned.prscs]), file = paste0(PennPRS_finalresults_path, trait_name,'.',method,'.optimal_params.txt'))
    if ( opt$verbose >= 1 ){
      print(paste0('*************************************'))
      print(paste0('******* Complete training ', method, ' *******'))
      print(paste0('*************************************'))
...
if (ensemble) save(ensemble.methods, err.CT, err.lassosum2, err.LDpred2, err.PRSCS, file = paste0(PennPRS_finalresults_path, 'step1.RData'))
if (!ensemble) save(err.CT, err.lassosum2, err.LDpred2, err.PRSCS, file = paste0(PennPRS_finalresults_path, 'step1.RData'))
