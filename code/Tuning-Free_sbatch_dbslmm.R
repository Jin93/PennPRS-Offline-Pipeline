...
option_list = list(
  ...
  make_option("--methods", action = "store", default = NA, type = "character",
              help="Options: a subset of methods from PRS-CS-auto, LDpred2-auto, and DBSLMM, divided by comma"),
...
if ('DBSLMM' %in% methods){
  ...
  if (file.exists(gwasinput)){
    ...
    ref = bigreadr::fread2(paste0(eval_ld_ref_path, LDrefpanel, '_hm3_', race, '_ref.bim'))[,c(2,4)]
  ...
  for (chr in 1:22) {
    ...
    # ## summary statistics
    # summstats <- bigreadr::fread2(paste0(prsdir, 'summary_gemma/chr',chr,'.assoc.txt'))
    ...
    # prefix_file <- paste0('chr',chr)
    # write.table(plink_summstats, file = paste0(prsdir, 'output/plink_', prefix_file, ".txt"),
    #             row.names = F, quote = F)
    # 
    # ## large effect clumping
    # clumping_cmd <- paste0(path_plink, ' --bfile ', eval_ld_ref_path, 'chr', chr, 
    #                        " --silent --clump ", prsdir, 'output/plink_', prefix_file, ".txt",
    #                        " --clump-r2 ", 0.2, " --clump-p1 ", 1e-6,
    #                        " --clump-kb ", 1000, 
    #                        " --out ", prsdir, 'output/l_', prefix_file)
    # system(clumping_cmd)
    
    filen<-paste0(prsdir, 'dbslmm', chr, '.sh')
    file.create(filen)
    zz <- file(filen, "w")
    cat(paste0('#!/bin/bash\n\n'), file = zz)
    cat(paste0('#$ -cwd\n\n'), file = zz)
    cat(paste0('#$ -o ', prsdir, 'logfile\n'), file = zz)
    cat(paste0('#$ -e ', prsdir, 'logfile\n\n'), file = zz)
    cat(paste0('dbslmm=', dbslmm_path, 'dbslmm\n'), file = zz)
    cat(paste0('chmod 777 ${dbslmm}\n'), file = zz)
    cat(paste0('let chr=', chr, '\n'), file = zz)
    cat(paste0('DBSLMM=', dbslmm_path, 'software/DBSLMM.R\n'), file = zz)
    cat(paste0('summf=', prsdir, 'summary_gemma/chr\n'), file = zz)
    # cat(paste0('summf=', dbslmm_path, 'test_dat/summary_gemma_chr\n'), file = zz)
    cat(paste0('outPath=', prsdir, 'output/\n'), file = zz)
    cat(paste0('plink=', plink_path, 'plink\n'), file = zz)
    # cat(paste0('ref=', eval_ld_ref_path, 'chr\n'), file = zz)
    # cat(paste0('ref=', eval_ld_ref_path, '1kg_hm3_', race, '_ref\n'), file = zz)
    cat(paste0('ref=', dbslmm_path, 'ref_data/ref_\n'), file = zz)
    cat(paste0('blockf=', dbslmm_path, 'block_data/', race.dbslmm, '/chr\n'), file = zz)
    cat(paste0('m=`cat ${summf}${chr}.assoc.txt | wc -l`\n'), file = zz)
    cat(paste0('herit=', ldsc_h2_est, '\n'), file = zz)
    cat(paste0('model=DBSLMM\n'), file = zz)
    cat(paste0('nobs=`sed -n "2p" ${summf}${chr}.assoc.txt | awk \'{print $5}\'`\n'), file = zz)
    cat(paste0('nmis=`sed -n "2p" ${summf}${chr}.assoc.txt | awk \'{print $4}\'`\n'), file = zz)
    cat(paste0('n=$(echo "${nobs}+${nmis}" | bc -l)\n\n'), file = zz)
    
    cat(paste0('Rscript ${DBSLMM} --summary ${summf}${chr}.assoc.txt --outPath ${outPath} --plink ${plink} --dbslmm ${dbslmm} --ref ${ref}${chr} --model ${model} --n ${n} --nsnp ${m} --block ${blockf}${chr}.bed --h2 ${herit} --thread ', NCORES,'\n\n'), file = zz)
    cat(paste0('wait\n'), file = zz)
    close(zz)
    
    # Create the .sh file for submitting dbslmm.sh
    filen<-paste0(prsdir, 'submit-dbslmm',chr,'.sh')
    file.create(filen)
    zz <- file(filen, "w")
    cat(paste0('#!/bin/bash\n\n'), file = zz)
    cat(paste0('cd ', prsdir, '\n\n'), file = zz)
    cat(paste0('sbatch -t 00:30:00 -A standby --mem=5G dbslmm',chr,'.sh\n\n'), file = zz)
    close(zz)
    system(paste0('sh ', prsdir, 'submit-dbslmm',chr,'.sh'))
