...
option_list = list(
  ...
  make_option("--N_THREADS", action = "store", default = 5, type = "numeric",
              help="Number of threads (default: 5) used for parallel computing of PRS-CS across chromosomes.
              Options: positive integer [Optional]"),
...
if ('MUSSEL' %in% methods){
    ...
    for (ite in 1:k){
      ...
      for (chr in 1:22){
        ...
        if(file.exists(output_mussel)){
          scoretemp = bigreadr::fread2(output_mussel)  # "rsid"    "a1"      "a0"
          n.tuning = ncol(scoretemp) - 2
          colnames(scoretemp) = c('SNP', 'A1', paste0('BETA',1:n.tuning))
          score = rbind(score, scoretemp); rm(scoretemp)
        }
      }
      # Match alleles with GWAS summary data:
      stateval = bigreadr::fread2(paste0(output_path, trait_name, '.gwas.ite', ite, '.txt'))
  ...
  for (race in races){
    ...
    for (chr in 1:22){
      ...
      if(file.exists(output_mussel)){
        score = bigreadr::fread2(output_mussel)  # "rsid"    "a1"      "a0"
        score[is.na(score)] = 0
        n.tuning = ncol(score) - 2
        colnames(score) = c('SNP', 'A1', paste0('BETA',1:n.tuning))
  ...
  # SNP.all = unique(unlist(lapply(1:length(beta_mussel), function(x){beta_mussel[[x]]$SNP})))
  # beta = NULL
  beta = beta_mussel[[1]][, c('CHR', 'SNP', 'A1', paste0('BETA', 1:n.tuning))]; anc = 1
  colnames(beta) = c(paste0('CHR.', anc), 'SNP', paste0('A1.', anc), paste0('BETA', (n.tuning*(anc-1) + 1) : (n.tuning*anc)))
  rownames(beta) = beta$SNP
  for (anc in 2:length(beta_mussel)){
    tem = beta_mussel[[anc]][, c('CHR', 'SNP', 'A1', paste0('BETA', 1:n.tuning))]
    colnames(tem) = c(paste0('CHR.', anc), 'SNP', paste0('A1.', anc), paste0('BETA', (n.tuning*(anc-1) + 1) : (n.tuning*anc)))
    rownames(tem) = tem$SNP
    # SNP.add = setdiff(SNP.all, tem$SNP)
    # tem.add = beta[SNP.add, ]
    beta = full_join(beta, tem, by = 'SNP')
    
    # Match alleles:
    na.ind = which(is.na(beta[, (n.tuning*(anc-1) + 1)]))
    if (length(na.ind) > 0){
      stateval[na.ind, paste0('BETA',1:n.tuning)] = 0
      stateval[na.ind,'A1'] = stateval[na.ind,'A1.ref']
      stateval[na.ind,'A2'] = stateval[na.ind,'A2.ref']
